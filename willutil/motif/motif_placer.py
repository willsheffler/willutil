...
def make_test_motif(xyz, sizes, minsep=0, minbeg=0, minend=0, ntries=3, rnoise=0, lever=10, nasym=None,
                    cbreaks=[0]):
   nres = len(xyz)
   nasym = nasym or nres
...
def check_offsets_overlap_containment(offsets, sizes, nres, nasym=None, cbreaks=None, minsep=0, minbeg=0,
                                      minend=0):
   offsets = torch.as_tensor(offsets)
   if offsets.ndim == 1: offsets = offsets.unsqueeze(1)
