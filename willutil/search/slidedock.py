...
def slide_dock_oligomer(sym, psym, nsym, xyzorig, step=1.0, clash_radius=2.4, contact_dis=8, startaxis=None):
    ...
    bvh = wuc.bvh.BVH(xyz[:, :, :].reshape(-1, 3))

    best = 0, None, None
    for i in range(nrot):
        pos1 = wu.hrot(paxis, i * step)
        pos2 = tonbr @ pos1
        delta = wuc.bvh.bvh_slide(bvh, bvh, pos1, pos2, clash_radius, slidedirn[:3])
        ...
        score = wuc.bvh.bvh_count_pairs_vec(bvh, bvh, pos1, pos2, contact_dis)
        if score > best[0]:
