...
def _debug_helper_show_close_xforms(
    reg1, reg2, xformsrand, targetcom, randaxis=None, randcen=None, thresh=1, showall=False
):
...
def perftest_motif_placer():
    ...
    _ = place_motif_dme_fast(
        xyz[:40], motif, nasym=nasym, cbreaks=cbreaks, junct=junct, nolapcheck=10, nrmsalign=10
    )
...
def test_motif_placer_minbeg_minend(showme=False):
    for minbeg in [0, 3]:
        for minend in [0, 4]:
            ...
            junct = 10
            sizes = [12, 13]
            motif, motifpos = make_test_motif(
                xyz, sizes, rnoise=0.1, nasym=nasym, cbreaks=cbreaks, minbeg=minbeg, minend=minend
            )
            fastdme = place_motif_dme_fast(
                xyz,
                motif,
                nasym=nasym,
                cbreaks=cbreaks,
            ...
            doffset, dme, alldo, alldme = place_motif_dme_brute(
                xyz, motif, nasym=nasym, cbreaks=cbreaks, minbeg=minbeg, minend=minend
            )
