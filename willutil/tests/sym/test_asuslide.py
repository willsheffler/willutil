...
def asuslide_case4():
    ...
    frames = wu.sym.frames(
        sym, ontop=primaryframes, cells=(-1, 1), cellsize=cellsize, center=cen, xtalrad=cellsize * 0.9
    )
...
def test_asuslide_helix_nfold1_2():
    ...
    rb2 = wu.sym.helix_slide(
        h, xyz, cellsize, contactfrac=0.3, closest=20, steps=30, step=8.7, iters=5, showme=False, **showmeopts
    )
...
def test_asuslide_I4132_clashframes():
    ...
    frames = wu.sym.frames(
        sym, ontop=primaryframes, cells=(-1, 1), cellsize=csize, center=wu.hcom(xyz), xtalrad=csize * 0.5
    )
    # frames = primaryframes
...
def test_asuslide_case2():
    ...
    assert np.allclose(
        slid.asym.com(), [1.81500000e01, -4.17462713e-04, 4.31305757e-15, 1.00000000e00], atol=0.1
    )
