...
def test_xtalfit_I213():
    ...
    vals = list()
    for i in range(10):
        fitsym = xtal.symcoords(asym + wu.hrandvec(), cells=0, cellsize=cell + np.random.normal()).reshape(
            -1, 4
        )
...
def DISABLED_test_xtalfit_I213_bk():
    ...
    if wu.hdot([1, 1, 1], ax3) < 0:
        ax3 = -ax3
    if wu.hdot([0, 0, 1], ax2) < 0:
        ax2 = -ax2
    # ic(ax3, ax2)
    # ic(cen3, cen2)
    ...
    vals = list()
    for i in range(100):
        fitsym = xtal.symcoords(coords[0] + wu.hrandvec(), cells=0, cellsize=cell + np.random.normal())[
            :, :, 1
        ].reshape(-1, 4)
        vals.append(wu.homog.hrmsfit(ref, fitsym)[0] - rfit)
    vals = np.array(vals)
