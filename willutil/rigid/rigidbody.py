...
class RigidBody:
    ...
    def __init__(
        ...
        **kw,
        ...
        if coords is not None:
            ...
            if usebvh:
                self.bvh = wuc.bvh.BVH(coords[..., :3])
                self.contactbvh = wuc.bvh.BVH(contact_coords[..., :3])
    ...
    def contact_count(self, other, contactdist, usebvh=None):
        ...
        if usebvh or (usebvh is None and self.usebvh):
            count = wuc.bvh.bvh_count_pairs(self.contactbvh, other.contactbvh, self.position, other.position,
                                            contactdist)
        else:
            assert 0
    ...
    def hasclash(self, other, clashdis=None):
        self.bvhopcount += 1
        clashdis = clashdis or self.clashdis
        isect = wuc.bvh.bvh_isect(self.bvh, other.bvh, self.position, other.position, clashdis)
        return isect

    def intersects(self, other, otherpos, mindis=10):
        self.bvhopcount += 1
        isect = wuc.bvh.bvh_isect_vec(self.bvh, other.bvh, self.position, otherpos, mindis)
        return isect
    ...
    def interactions(self, other, contactdist=8, buf=None, usebvh=None):
        ...
        if usebvh or (usebvh is None and self.usebvh):
            ...
            pairs, overflow = wuc.bvh.bvh_collect_pairs(self.contactbvh, other.contactbvh, self.position,
    ...
    def clash_interactions(self, other, contactdist=8, buf=None, usebvh=None):
        ...
        if usebvh or (usebvh is None and self.usebvh):
            if not buf: buf = np.empty((100000, 2), dtype="i4")
            pairs, overflow = wuc.bvh.bvh_collect_pairs(self.bvh, other.bvh, self.position, other.position,
                                                        contactdist, buf)
            assert not overflow
        else:
            d = wu.hnorm(self.coords[None] - other.coords[:, None])
