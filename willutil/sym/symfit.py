...
def compute_symfit(
    ...
    **kw,
    ...
    if len(frames) > len(wu.sym.sym_frames[sym]):
...
def get_redundant_cyclic_err(
    ...
    **kw,
    ...
    # print(maxangrad)
    # wu.showme(symops.axs)
    # wu.showme(symops.cen)
    # print(symops.axs.shape)
    angs = wu.line_angle(symops.axs, symops.axs, outerprod=True)
...
def xform_update_symop(symop, xform, srad):
    ...
    # wu.showme([p, np.array([0, 0, 0, 1]), result])
    # assert 0
...
def disambiguate_axes(sym, axis, nfold, noambigaxes=True, **kw):
    if wu.sym.ambiguous_axes(sym) is None:
        return nfold
    if not noambigaxes:
        return nfold
    nfold1 = nfold.copy()
...
def get_symops_isect(sym, cen1, cen2, axs1, axs2, nfold, max_nan=0.9, isect_outliers_sd=3, **kw):
    ...
    not_same_symaxis = axes_angles > wu.sym.minsymang[sym]
...
def symops_align_axes(
    ...
    **kw,
    ...
    if choose_closest_frame:
        # assert 0
        axes_perm_choices = wu.sym.sym_permute_axes_choices(sym)
        choices = axes_perm_choices[:, None] @ wu.sym.sym_frames[sym][None, :]
        choices = choices.reshape(-1, 4, 4)
        # print(xfit)
        pos = (choices @ xfit @ frames[0])[:, :3, 3]
        # print(pos)
        dots = wu.homog.hdot([3, 2, 1], pos)
        # print(dots)
    ...
    #     col = [0, 0, 0]
    #     col[i] = 1
    #     wu.showme(ax, col=col, usefitaxis=True, name='sopA')
    #     # wu.showme(tgtaxes[i], col=col, usefitaxis=True, name='tgtA')
    #     ax = hm.hxform(xfit, ax)
    #     wu.showme(ax, col=col, usefitaxis=True, name='nfoldB')
...
def symfit_mc_play(
    ...
    **kw,
    ...
    # wu.showme(frames, 'start', col=(1, 1, 1), **showme_opts)
    ...
    # wu.showme(wu.sym.sym_frames[kw.sym][:, None] @ frames[None, :], name='symstart',
    ...
    for isamp in range(maxiters):
        ...
        # wu.showme(symdupframes, name='xfitmc%05i' % isamp, col=None, **showme_opts)
        ...
        if np.exp(-delta / temp) > np.random.rand():
            ...
            if showme and delta < 0 and isamp - lastviz > vizinterval:
                ...
                if showsymdups:
                    ...
                    symdupframes = frames
                    wu.showme(
                        symdupframes, name="xfitmc%05i" % isamp, col=None, **showme_opts.sub(vizfresh=vizfresh)
                    )
                ...
                # wu.showme(pairs, name='mc%05i' % isamp, col='bycx', center=[0, 0, 0],
                # **showme_opts)
                lastviz = isamp
    ...
    if False:
        ...
        pairs = wu.sym.stupid_pairs_from_symops(
            symfit.symops,
        )
    ...
    # assert 0
    # wu.showme(frames, name='best', col=(1, 1, 1), center=[0, 0, 0], **showme_opts)
    # wu.showme(symdupframes, name='xfitbest', col=(0, 0, 1), rays=0.1, **showme_opts)

    # t.report()

    return wu.Bunch(nsamp=isamp + 1, besterr=besterr, symerr=symerr, frames=frames, start_err=err0)
