    ...
    for nfold in [2, 3, 4, 6, -1, -2, -3, -4, -6]:
        ...
        for tag in keep:
            try:
                ...
                se = SymElem(
                    nfold, axs_, cen_, hel=hel_, lattice=lattice, isunit=True, latticetype=latticetype
                )
...
def _shift_to_unitcell(symelems):
    ...
    for psym, elems in symelems.items():
        ...
        for e in elems:
            ...
            newelems.append(
                SymElem(e.nfold, e.axis, cen, hel=e.hel, screw=e.screw, frame=e.frame, isunit=True)
            )
            # ic(e)
...
def _symelem_remove_ambiguous_syms(symelems):
    symelems = symelems.copy()
    for sympair in (
        "c2/c4 c2/c6 c3/c6 c11/c21 c11/c31 c11/c32 c11/c41 c11/c42 c11/c43 "
...
def _remove_redundant_screws(symelems, frames, lattice):
    ...
    for psym, elems in symelems.items():
        ...
        for ie, eunit in enumerate(elemssort):
            ...
            if debug:
                wu.showme(symcen, scale=12, name=f"cen_{ie}")
            if debug:
                wu.showme(e, scale=12, name=f"elem_{ie}")
            for ie2, e2unit in enumerate(newelems):
                e2 = e2unit.tolattice(lattice)
                if debug:
                    wu.showme(e2, scale=12, name=f"other_{ie}{ie2}")
