...
def asuslide(
    ...
    **kw,
    ...
    assembly = wu.rigid.RigidBodyFollowers(
        sym=sym, coords=coords, frames=frames, recenter=True, cellsize=cellsize, **kw
    )
    if showme:
        wu.showme(assembly, name="START", **kw)
    ...
    for i in range(iters):
        ...
        for j in range(subiters):
            ...
            for iax, (axis, axpos) in enumerate(axes):
                ...
                if towardaxis:
                    # ic(axpos)
                    axisperp = wu.hnormalized(
                        wu.hprojperp(axis, assembly.asym.com() - axpos - wu.hrandvec() / 1000)
                    )
    ...
    if showme:
        wu.showme(assembly, name="FINISH", **kw)
    return assembly
...
def slide_axis(
    ...
    **kw,
    ...
    for i in range(maxstep):
        ...
        total += flip * step
        if showme:
            wu.showme(assembly, name="slideaxis%f" % axis[0], **kw)
    ...
    if not success and resetonfail:
        ...
        if showme:
            wu.showme(assembly, name="resetaxis%f" % axis[0], **kw)
        if timer:
    ...
    if iflip == 0:  # back off so no clash
        ...
        if showme:
            wu.showme(assembly, name="backoffaxis%f" % axis[0], **kw)
...
def slide_cellsize(
    ...
    **kw,
    ...
    if showme:
        wu.showme(assembly, name="scaleinput", **kw)
    ...
    for i in range(maxstep):
        ...
        if not boundscheck(assembly):
            break
        if showme:
            wu.showme(assembly, name="scale", **kw)
    ...
    if not success:
        ...
        if showme:
            wu.showme(assembly, name="resetscale", **kw)
    ...
    if iflip == 0 and success:  # back off
        ...
        if showme:
            wu.showme(assembly, name="backoffscale", **kw)
...
def recenter_asu_frames(
    ...
    **kw,
    ...
    if symelemsibs is None:
        ...
        if showme:
            wu.showme(assembly, name="recenasuabs", **kw)
        return
