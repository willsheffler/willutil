...
def check_if_symelems_complete(spacegroup, symelems, depth=60, radius=5, trials=100000, fudgefactor=0.9):
   ...
   frames, _ = wu.cpp.geom.expand_xforms_rand(generators, depth=depth, radius=radius, trials=trials)
...
def minimal_spacegroup_cover_symelems(spacegroup, maxelems=5, noscrew=False, nocompound=False):
   ...
   if noscrew: allsymelems = [e for e in allsymelems if not e.isscrew]
   if nocompound: allsymelems = [e for e in allsymelems if not e.iscompound]
   max_combo = min(len(allsymelems), maxelems)
   wu.printheader('compute covers', spacegroup, len(allsymelems), max_combo, padstart=10)
