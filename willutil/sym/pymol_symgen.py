...
def makedx(sel="all", n=2, name=None):
    ...
    for i in range(n):
        ...
        for ic, c in enumerate(chains):
            ...
            cmd.alter(
                "((%s) and chain %s )" % (dsel2, c), "chain = '%s'" % ALLCHAIN[len(chains) * (i + n) + ic]
            )
...
def make_d3oct(d3, cage, cage_trimer_chain="A", depth=4, maxrad=9e9):
    if verbose:
        print(
            cmd.super("((" + cage + ") and (chain " + cage_trimer_chain + "))", "((" + d3 + ") and (chain A))")
        )
    ...
    def __call__(self, node, **kwargs):
        ...
        for icen, cen in enumerate(self.nodes):
            ...
            if self.bounds_check(xcen):
                self.add_sphere(
                    x * (cen + Vec(0, 0, 0)), 2.0, text="%s%i" % ("ABCD"[icen], node.depth), icol=icen
                )
                self.add_sphere(
                    x * (cen + Vec(2, 0, 0)), 2.0, text="%s%i" % ("ABCD"[icen], node.depth), icol=icen
                )
                self.add_sphere(
                    x * (cen + Vec(0, 2, 0)), 2.0, text="%s%i" % ("ABCD"[icen], node.depth), icol=icen
                )
            pcen = xcen
