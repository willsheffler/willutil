        ...
        for head, xpos in itertools.chain(newheads, heads):
            ...
            for seennode, seenframe in itertools.chain(newnewheads, newheads, heads, body):
                if candidate == seenframe:
                    ...
                    break
            if not seenit:
                newhead = SymTrieNode(generators, ielem, iframe, head.depth + 1,
                                      position=candidate)
...
def mofview():
    ...
    cmd.show('sti',
             'resn asp+das+cys+dcs+his+dhi+glu+dgu+zn+bpy and not hydro and not name n+c+o')
            ...
            for n in CClist:
                ...
                if showframes and ipn < len(component_pos):
                    for stn in self.priCCtoCCframes[pn][n]:
                        ...
                        if self.showlinks:
                            CGO.extend(
                                cgo_cyl_arrow(n, cn, rad=0.3, col=self.colors[ipn], arrowlen=2.0))
        v = cmd.get_view()
        cmd.delete(self.label)
    ...
    def make_symdef(self, **kwargs):
        ...
        for ip, val in enumerate(self.priCCtoCCframes.items()):
            ...
            for icc, val2 in enumerate(CCframes.items()):
                ...
                if True:
                    Sxyz += (XYZ_TEMPLATE %
                             (CCDofBegName, ELEMDIR.x, ELEMDIR.y, ELEMDIR.z, ELEMDIR2.x,
                              ELEMDIR2.y, ELEMDIR2.z, CC.x * scale, CC.y * scale, CC.z * scale))
                if True:
                    Sxyz += (XYZ_TEMPLATE %
                             (CCDofEndName, ELEMDIR.x, ELEMDIR.y, ELEMDIR.z, ELEMDIR2.x,
                              ELEMDIR2.y, ELEMDIR2.z, CC.x * scale, CC.y * scale, CC.z * scale))
                if PCC:
                ...
                for isub, stn in enumerate(STNs):
                    ...
                    Sxyz += (XYZ_TEMPLATE % (SUBName, SX.x, SX.y, SX.z, SY.x, SY.y, SY.z,
